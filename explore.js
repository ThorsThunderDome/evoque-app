// explore.js - FIXED INFINITE LOADING BUG
...
async function buildExploreFeed() {
    ...
    try {
        ...
        postsSnapshot.forEach(doc => {
            ...
            if (subscribedCreatorIds.includes(post.creatorId)) subscribedPosts.push(post);
            else discoverPosts.push(post);
        });
        
        subscribedContainer.innerHTML = subscribedPosts.length === 0 ? "<p>Posts from your subscribed creators will appear here.</p>" : '';
        subscribedPosts.forEach(post => subscribedContainer.appendChild(createPostCard(post, true)));
        ...
        if (creators.length > 0) {
            ...
            let showcaseHTML = `
                ...
                <div class="horizontal-scroll-wrapper">
                    <button class="scroll-arrow left" aria-label="Scroll Left"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
                    <div class="horizontal-scroll">`;
            
            creators.forEach(creator => {
                // --- THIS IS THE FIX ---
                // We now check if creator.bio exists before trying to shorten it.
        ...
        discoverContainer.innerHTML = discoverPosts.length === 0 ? "<p>No other posts to discover right now.</p>" : '';
...
// CRITICAL FIX: Make sure the app is ready before trying to build the feed
